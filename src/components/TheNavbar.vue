      ...
      <div class="collapse navbar-collapse" :class="{ show: isMenuOpen }">
        ...
        <div class="d-flex align-items-center gap-2 ms-xl-auto">
          <template v-if="!isLoggedIn">
            ...
            <router-link
              to="/login"
              class="btn btn-warning px-3 rounded-pill fw-bold"
              @click="closeMenu"
            >
              Đăng nhập
            </router-link>
          ...
          <template v-else>
            <div class="user-dropdown">
              <button
                class="avatar-btn d-flex align-items-center gap-2"
              ...
              >
                ...
                <i
                  class="bi bi-chevron-down chevron-icon"
              ...
              <Transition name="dropdown">
                <div v-if="isDropdownOpen" class="dropdown-menu-custom shadow">
                  ...
                  <ul class="list-unstyled mb-0 py-2">
                    ...
                    <li>
                      <button
                        class="dropdown-item-custom logout-btn w-100 text-start"
                        @click="handleLogout"
                      >
...
const userRole = computed(() => {
  ...
  return decodedPayload.value["role"] || 
...
const avatarLetter = computed(() =>
  userDisplay.value.charAt(0).toUpperCase() || "U"
);
...
<style scoped>
/* Toàn bộ CSS Navbar của mi giữ nguyên, cực kỳ đẹp! */
.search-box { max-width: 500px; }
.search-box input::placeholder { color: rgba(255, 255, 255, 0.6); }
.nav-link { font-weight: 500; transition: 0.3s; }
.nav-link.router-link-active { color: #ffc107 !important; }
.btn-warning { transition: transform 0.2s; }
.btn-warning:hover { transform: translateY(-2px); box-shadow: 0 4px 10px rgba(255, 193, 7, 0.3); }
.avatar-btn { background: none; border: none; cursor: pointer; padding: 4px 8px; border-radius: 50px; transition: background 0.2s; }
.avatar-btn:hover { background: rgba(255, 255, 255, 0.08); }
.avatar-circle { width: 38px; height: 38px; border-radius: 50%; background: linear-gradient(135deg, #ffc107, #ff8c00); display: flex; align-items: center; justify-content: center; font-weight: 700; font-size: 1rem; color: #121212; flex-shrink: 0; border: 2px solid rgba(255, 193, 7, 0.4); box-shadow: 0 0 12px rgba(255, 193, 7, 0.25); }
.avatar-circle.avatar-lg { width: 46px; height: 46px; font-size: 1.2rem; }
.chevron-icon { color: rgba(255, 255, 255, 0.6); font-size: 0.75rem; transition: transform 0.3s ease; }
.chevron-icon.rotated { transform: rotate(180deg); }
.user-dropdown { position: relative; }
.dropdown-backdrop { position: fixed; inset: 0; z-index: 998; }
.dropdown-menu-custom { position: absolute; top: calc(100% + 12px); right: 0; min-width: 240px; background: #1e1e1e; border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 14px; overflow: hidden; z-index: 999; }
.dropdown-item-custom { display: flex; align-items: center; padding: 10px 20px; color: rgba(255, 255, 255, 0.85); font-size: 0.88rem; text-decoration: none; transition: background 0.2s, color 0.2s; background: none; border: none; cursor: pointer; }
.dropdown-item-custom:hover { background: rgba(255, 193, 7, 0.08); color: #ffc107; }
.logout-btn { color: #ff6b6b; }
.logout-btn:hover { background: rgba(255, 107, 107, 0.08); color: #ff4444; }
