<template>
  <div class="d-flex flex-column h-100 bg-light position-relative overflow-hidden">
    <div class="p-2 bg-white border-bottom shadow-sm flex-shrink-0 z-2">
      <div class="d-flex gap-2 align-items-center">
        <div class="input-group input-group-sm rounded-pill overflow-hidden border flex-grow-1 bg-light">
        ...
        <button
          ...
          :class="selectedCategoryIds.length > 0 ? 'btn-primary shadow-sm' : 'btn-outline-secondary'"
          @click="showFilter = true"
        >
          <i class="bi bi-funnel-fill me-1"></i> Lọc
          <span v-if="selectedCategoryIds.length > 0" class="badge bg-danger ms-2 rounded-pill">
        ...
        >
          <i class="bi me-1" :class="viewMode === 'grid' ? 'bi-list-ul' : 'bi-image'"></i>
    ...
    <div class="flex-grow-1 overflow-auto p-3 custom-scrollbar z-1">
      <div
        v-if="viewMode === 'grid'"
        class="row row-cols-3 row-cols-sm-4 row-cols-md-4 row-cols-xl-5 gx-2 gy-4"
      >
        <div class="col" v-for="product in filteredProducts" :key="'grid-' + product.id">
          ...
          >
            ...
            <div class="card-body p-2 text-center d-flex flex-column justify-content-between bg-white">
              <h6 class="card-title text-dark fw-bold mb-1 product-name lh-sm" :title="product.name">
                {{ product.name }}
              </h6>
              <span class="text-primary fw-bolder mt-auto">{{ formatVND(product.price) }}</span>
            </div>
          </div>
        </div>
      </div>

      <div v-else class="row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-xl-4 g-2">
      ...
      <div v-if="filteredProducts.length === 0" class="col-12 py-5 text-center text-muted w-100">
    ...
    >
      <div class="p-3 border-bottom d-flex justify-content-between align-items-center bg-light">
        ...
        <button class="btn-close" style="font-size: 0.75rem" @click="showFilter = false"></button>
      </div>
      ...
      <div class="flex-grow-1 overflow-auto p-3 custom-scrollbar">
        <div class="form-check mb-3">
          ...
          />
          <label class="form-check-label fw-semibold cursor-pointer w-100" for="cat-all"
            >Tất cả nhóm hàng</label
          >
        </div>
        <hr class="text-muted opacity-25" />

        <div class="form-check mb-3" v-for="cat in uniqueCategories" :key="cat.id">
          <input
            class="form-check-input cursor-pointer"
            type="checkbox"
            :value="cat.id"
          ...
          />
          <label class="form-check-label cursor-pointer w-100 text-secondary" :for="'cat-' + cat.id">
            {{ cat.name }}
          </label>
        </div>
      </div>
      ...
      <div class="p-3 border-top d-flex gap-2">
        ...
        <button class="btn btn-sm btn-primary fw-bold flex-grow-1" @click="showFilter = false">
          Áp dụng
        </button>
      </div>
    </div>
